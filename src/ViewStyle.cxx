...
FontRealised::FontRealised(bool styleAutoComplete) {
...
void FontRealised::Realise(Surface &surface, int zoomLevel, int technology, const FontSpecification &fs) {
  ...
  lineHeight = static_cast<unsigned int>(surface.Height(font));
...
void ViewStyle::Refresh(Surface &surface, int tabInChars) {
	...
	CreateAndAddFont(styles[STYLE_DEFAULT], false);
	for (unsigned int j=0; j<styles.size(); j++) {
		CreateAndAddFont(styles[j], j == STYLE_AUTOCOMPLETION);
	}
...
void ViewStyle::CreateAndAddFont(const FontSpecification &fs, bool styleAutoComplete) {
	if (fs.fontName) {
		...
		if (it == fonts.end()) {
			fonts[fs] = std::unique_ptr<FontRealised>(new FontRealised(styleAutoComplete));
		} else {
...
void ViewStyle::FindMaxAscentDescent() {
	for (FontMap::const_iterator it = fonts.cbegin(); it != fonts.cend(); ++it) {
		if (maxAscent < it->second->ascent && !it->second->styleAutoCompleteOnly)
			maxAscent = it->second->ascent;
		if (maxDescent < it->second->descent && !it->second->styleAutoCompleteOnly)
